...
import { useAllClientsQuery } from "@/tanstack_query/hooks/useAllClientsQuery";
...
export default function ClientsPanel() {
	...
	const { data, isFetching, isError } = useAllClientsQuery();

	if (isError) {
		return <div className="rounded bg-red-700 p-2 text-lg text-white">Failed to fetch clients</div>;
	}

	const initClientCreation = () => {
		...
		<div className="flex">
			<div className="flex w-1/4 flex-col">
				<div
					...
					onClick={() => initClientCreation()}
				>
					<PlusIcon />
					<p className="pl-2 text-lg">New Client</p>
				</div>
			...
			{/* <div className="flex w-3/4 pl-3">{selectedClient && <ClientDetailsPanel client={selectedClient} />}</div> */}
						...
						<div className="flex min-h-full items-center justify-center p-4 text-center">
							...
							>
								<Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-lg shadow transition-all">
									<ClientEditForm client={selectedClient} onCancel={cancelEditForm} onDone={onClientSaved} />
								</Dialog.Panel>
							</Transition.Child>
						</div>
					</div>
