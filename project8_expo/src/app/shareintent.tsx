...
const WebUrlComponent = ({ shareIntent }: { shareIntent: ShareIntentType }) => {
	return (
		<View style={[styles.gap, styles.row, { borderWidth: 1, borderRadius: 5, height: 102 }]}>
			<Image
				source={shareIntent.meta?.["og:image"] ? { uri: shareIntent.meta?.["og:image"] } : undefined}
...
export default function ShareIntent() {
	...
	return (
		<View style={styles.container}>
			...
			{hasShareIntent && (
				<Text style={[styles.gap, { fontSize: 20 }]}>Congratz, a share intent value is available</Text>
			)}
